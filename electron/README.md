...
- Linux: `~/.config/clipforge-desktop/clipforge.db` (+ `projects/`, `models/`)
- macOS: `~/Library/Application Support/clipforge-desktop/`
- Win: `%APPDATA%\clipforge-desktop\`
...
```bash
# download for this machine's tier into its userData (~/.config/clipforge-desktop/models)
...
- Models live at `<userData>/models/...` (`~/.config/clipforge-desktop/models/` on Linux). Override `USER_DATA_PATH` to stage elsewhere. The installer can pre-seed `resources/models/` and copy on first run (see `electron-builder.yml:15`).
- To force re-download: `rm -rf ~/.config/clipforge-desktop/models` then `npm run setup:models`.
...
LICENSE_VERIFY_URL=https://clipforge.com/api/license/verify
```
...
Builder config: `electron-builder.yml` (`appId com.clipforge.desktop`). Binaries under `resources/bin/<plat>-<arch>/` are auto-included as `extraResources/bin`.
...
- **DB locked / JSON fallback**: Node <22.5 can't load `node:sqlite` → JSON file at `clipforge.json` is used (`src/services/db.ts:278`). Upgrade Node or set `USER_DATA_PATH` to a writable dir.
